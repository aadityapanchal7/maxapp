...
async def build_agent_system_prompt(
    ...
    chat_prompt += (
        ...
        "2. Quote each `[Name](https://www.amazon.com/dp/...)` markdown "
        "link VERBATIM. Do not paraphrase, shorten, or alter the URL.\n"
        "3. NEVER emit a search URL ('amazon.com/s?k=', 'google.com/"
        "search', etc.) — these are forbidden.\n"
        "4. NEVER invent ASIN paths or guess URLs.\n"
        "5. If the catalog block is empty, mention the ingredient/"
        "category by name (e.g. 'a 5% minoxidil foam') WITHOUT a URL "
        "and tell the user you don't have a vetted link for that yet.\n"
        "6. If the user has dietary or sensitivity facts (vegan, "
